...
func buildSinks(ctx context.Context, streamName string, outputs []config.SinkConfig) ([]plugin.Sink, error) {
	...
	for _, out := range outputs {
		...
		sink, err := pkgsink.Build(ctx, pkgsink.SinkConfig{
			...
			LokiURL:           out.LokiURL,
			LokiLabels:        out.LokiLabels,
			LokiBatchSize:     out.LokiBatchSize,
			LokiFlushInterval: out.LokiFlushInterval,
			LokiTenantID:      out.LokiTenantID,
			LokiUsername:      out.LokiUsername,
			LokiPassword:      out.LokiPassword,
			LokiGzip:          out.LokiGzip,
			LokiTLSCert:       out.LokiTLSCert,
			LokiTLSKey:        out.LokiTLSKey,
			LokiCACert:        out.LokiCACert,

			SplunkURL:           out.SplunkURL,
			SplunkToken:         out.SplunkToken,
			SplunkSourceType:    out.SplunkSourceType,
			SplunkSource:        out.SplunkSource,
			SplunkIndex:         out.SplunkIndex,
			SplunkHost:          out.SplunkHost,
			SplunkBatchSize:     out.SplunkBatchSize,
			SplunkFlushInterval: out.SplunkFlushInterval,
			SplunkGzip:          out.SplunkGzip,
			SplunkTLSCert:       out.SplunkTLSCert,
			SplunkTLSKey:        out.SplunkTLSKey,
			SplunkCACert:        out.SplunkCACert,

			DatadogURL:           out.DatadogURL,
			DatadogAPIKey:        out.DatadogAPIKey,
			DatadogSource:        out.DatadogSource,
			DatadogTags:          out.DatadogTags,
			DatadogHostname:      out.DatadogHostname,
			DatadogService:       out.DatadogService,
			DatadogBatchSize:     out.DatadogBatchSize,
			DatadogFlushInterval: out.DatadogFlushInterval,
			DatadogGzip:          out.DatadogGzip,
			DatadogTLSCert:       out.DatadogTLSCert,
			DatadogTLSKey:        out.DatadogTLSKey,
			DatadogCACert:        out.DatadogCACert,
		})
		if err != nil {
			return nil, fmt.Errorf("sink %q: %w", out.Type, err)
		}
