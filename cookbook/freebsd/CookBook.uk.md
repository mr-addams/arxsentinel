...
через goreleaser, плюс виділений інсталятор та rc.d-скрипт сервісу.
Є два способи запустити сам ArxSentinel на FreeBSD:

1. **Нативний бінарник на хості (рекомендовано, найкраще перевірено)** —
   через інсталятор нижче. Якщо ваш веб-сервер (nginx, Caddy, Traefik,
   HAProxy, Apache, LiteSpeed...) працює в контейнері `podman` на тому ж
   хості FreeBSD, ArxSentinel читає логи доступу контейнера через
   змонтований шлях на хості або через мережеве джерело (syslog/HTTP),
   точно як у будь-якому іншому рецепті цієї книги.
2. **По-справжньому FreeBSD-нативний образ контейнера** (ранній доступ —
   див. [Образ контейнера](#образ-контейнера) нижче) — зібраний на базі
   `ghcr.io/freebsd/freebsd-static`, справжнього FreeBSD OCI-образу. Це
   НЕ той самий шлях Linux-емуляції, який `podman` використовує для
   образів веб-серверів: ОС контейнера збігається з хостом FreeBSD, тому
   він працює як справжній `ocijail`-jail без жодного шару
   Linux-трансляції.

Чому нативний бінарник досі рекомендується за замовчуванням: це
архітектура, яку власний FreeBSD CI-набір цього проекту
(`tests/integration-freebsd/`) перевіряє повністю end-to-end, і вона
повністю обходить навіть шорсткості, що залишились у FreeBSD-нативного
образу контейнера (див. нижче) — взагалі без залежності від контейнерного
рантайму. Образ контейнера — легітимний, реальний варіант для тих, хто
хоче однорідну контейнерну історію деплою між Linux та FreeBSD, а не
обхідний шлях через Linux-емуляцію (це був би Варіант 3, від якого ця
книга явно застерігає — див.
[Запуск веб-серверів у podman на FreeBSD](#запуск-веб-серверів-у-podman-на-freebsd),
чому Linux-емульовані контейнери — принципово інший, гірше підтримуваний
шлях порівняно з обома варіантами вище).
...
ArxSentinel також публікує по-справжньому FreeBSD-нативний OCI-образ
контейнера — не Linux-образ, що працює через шар Linux-сумісності
`podman`. Образ збирається з `GOOS=freebsd GOARCH=amd64` та пакується
поверх `ghcr.io/freebsd/freebsd-static` (справжній FreeBSD OCI-образ,
що публікується самим проектом FreeBSD). Оскільки ОС контейнера
збігається з хостом FreeBSD, він працює як звичайний `ocijail`-jail —
жодного `--os=linux`, жодного шару Linux-емуляції, жодна з проблем із
[наступного розділу](#запуск-веб-серверів-у-podman-на-freebsd)
до цього образу не застосовується.
...
Версійовані теги також публікуються як `:<release-tag>-freebsd-amd64`
(наприклад `:v2.2.2-dev.12-freebsd-amd64`) поряд із плаваючим
`:freebsd-amd64-latest`.

**Поточні обмеження, про які варто знати перед вибором цього варіанту
замість нативного бінарника:**
- **Лише amd64.** Поки немає збірок під 386/arm/arm64 — збірка йде
  всередині єдиного amd64 гостя `vmactions/freebsd-vm` (QEMU) (у podman
  на FreeBSD немає аналога крос-архітектурного `buildx`), тому немає
  multi-arch маніфесту, з якого можна було б обрати іншу архітектуру.
- **Працює від root.** У базового образу `freebsd-static` немає
  попередньо встановленого непривілейованого користувача (на відміну
  від Linux-образу з `distroless:nonroot`), а встановлення довільного
  `USER` призвело б до uid, відсутнього в `/etc/passwd` образу —
  `ocijail` відмовляється запускати контейнер під невідомим uid.
  Посилення безпеки (nonroot) — відома задача на майбутнє, поки не
  зроблено.
- **Перевірено в CI, але ще не довго обкатано.** Пайплайн збірки
  доведений (4+ зелених прогони поспіль на момент написання, після
  того як більш ранній цикл налагодження виявив і виправив проблему з
  прапорцем збірки `CGO_ENABLED`), але це новіший, менш перевірений
  боєм шлях, ніж інсталятор з нативним бінарником вище, який якраз і
  перевіряє end-to-end власний FreeBSD-набір інтеграційних тестів цього
  проекту.

Якщо жодне з цих обмежень не критичне для вашої інфраструктури, це
легітимний спосіб отримати однорідну історію деплою
«все-в-контейнерах» між Linux та FreeBSD хостами, без накладних витрат
і проблем Linux-емуляції.
