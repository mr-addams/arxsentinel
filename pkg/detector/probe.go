...
func init() {
	Register("probe", newProbeFactory)
}
...
func newProbeFactory(cfg DetectorConfig, _ SharedResources) (plugin.Detector, error) {
	score := getInt(cfg.Params, "score", 25)
	paths := getStrings(cfg.Params, "paths", defaultProbePaths())
