...
func TestRateDetector_ViaRegistry(t *testing.T) {
	...
	result := d.Detect(highRate, &plugin.Event{Payload: &parser.LogEntry{}})
	if result.Score == 0 {
	...
	result2 := d.Detect(lowRate, &plugin.Event{Payload: &parser.LogEntry{}})
	if result2.Score != 0 {
...
func TestRateDetector_ValidParams(t *testing.T) {
	...
	result := d.Detect(svLow, &plugin.Event{Payload: &parser.LogEntry{}})
	if result.Score != 0 {
	...
	result = d.Detect(svHigh, &plugin.Event{Payload: &parser.LogEntry{}})
	if result.Score != 25 {
