...
func TestUADetector_ExtraPatternNormalization(t *testing.T) {
	...
	t.Run("mixed_case_extra_detected", func(t *testing.T) {
		...
		for _, tc := range cases {
			result := d.Detect(sv, &plugin.Event{Payload: &parser.LogEntry{UserAgent: tc.ua}})
			if result.Score == 0 {
	...
	t.Run("reason_uses_normalized_lowercase", func(t *testing.T) {
		...
		result := d.Detect(sv, &plugin.Event{Payload: &parser.LogEntry{UserAgent: "MyBot/2.0"}})
...
func TestUADetector_ExtraPatternNoMatchAfterNormalization(t *testing.T) {
	...
	result := d.Detect(sv, &plugin.Event{Payload: &parser.LogEntry{UserAgent: "RareBot/1.0"}})
	if result.Score == 0 {
