...
//
// Идея: каждый executor получал свою локальную map с одинаковой
// логикой очистки по TTL. Это приводило к дублированию кода и разному
// поведению между executor-ами. Decision 4 флоу 061 фиксирует единый
// пакет pkg/dedup.
//
// API:
...
//     upstream (внешний API executor-а) не оставляли ключ заблокированным
//     на TTL.
...
//	Mutex на уровне вызывающего (см. caller-side flush в продукте) —
//	пакет dedup не пытается решить эту задачу внутри, чтобы сохранить
...
// операций (например, executor flush), должен пометить все ключи в окне
